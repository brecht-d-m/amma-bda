...
# Starts a DataLab container deployed to a GCE VM.
...
if [ "$#" = "1" ] && [ "$1" = "--help" ]; then
  HELP_INFO="Google Cloud DataLab"
fi
...
if [ "$HELP_INFO" != "" ]; then
  ...
  echo "  vm name: the name of the VM to create (default: datalab)"
...
NOTEBOOKS_SOURCE="gs://cloud-datalab/content/"
PORT=8092
URL="http://localhost:$PORT"

echo "Starting DataLab (version $TAG) setup on VM '$VM' ($VM_TYPE) ..."
...
if [ $? = 0 ]; then
  ...
  echo "Or browse to $URL (if the process is also a DataLab run)."
...
# Wait for the DataLab container to start and become accessible
echo "Waiting for DataLab container to start ..."
until (gcloud -q compute ssh --command "sudo docker ps" $VM 2> /dev/null | grep -q datalab); do
  sleep 2
  printf "."
done
printf "\n"
...
# Wait for DataLab container to start and become accessible
echo "Waiting for DataLab server to start ..."
...
# Open DataLab in local browser session
